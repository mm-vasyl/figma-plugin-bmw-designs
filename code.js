...
const config = {
    ...
    gap: 20,
    document: figma.currentPage.parent,
...
figma.ui.onmessage = (msg) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (msg.type === 'import-data') {
        try {
            ...
            let prevWidth = width, prevHeight = height, x = 0 - prevWidth - gap, y = 0 - prevHeight - gap;
            // load Fonts
            yield figma.loadFontAsync({ family: "BMWTypeNext Pro", style: "Bold" });
            yield figma.loadFontAsync({ family: "BMWTypeNext Pro", style: "Light" });
            yield figma.loadFontAsync({ family: "BMWTypeNext Pro", style: "Regular" });
            // MAIN LOOP
            for (let i = 0; i < msg.data.length; i++) {
                const data = msg.data[i];
                if (!data['size'])
                    continue; // skip on empty rows
        ...
        if (!selection.length)
            return figma.ui.postMessage({ type: 'no-selection' });
        try {
            // load Fonts
            yield figma.loadFontAsync({ family: "BMWTypeNext Pro", style: "Bold" });
            yield figma.loadFontAsync({ family: "BMWTypeNext Pro", style: "Light" });
            ...
            for (let index = 0; index < selection.length; index++) {
                ...
                if (!data)
                    return figma.ui.postMessage({ type: 'no-element', name: selected.name });
                // get variant
                const variant = getVariant(data);
                const sizeWrapper = variant.createInstance();
                fillElementByData(sizeWrapper, data);
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (const element of config.fillConfig) {
                if (element[2] == 'image') {
                    ...
                    if (!image)
                        return figma.ui.postMessage({ type: 'read-error', name });
...
function getWidthAndHeight(size) {
    ...
    if (data == null)
        throw "Invalid size format in the spreadsheet.";
    return {
        width: Number(data[1]),
...
function replaceAllRegex(str, find, replace) {
...
function getNextPos(prevWidth, prevHeight, width, height, gap, frame, x, y) {
...
function isJsonCorrect(json) {
    const keys = config.requiredFields;
    for (let i = 0; i < keys.length; i++) {
        for (let j = 0; j < json.length; j++) {
            const prop = json[j].hasOwnProperty(keys[i]);
            if (!prop)
                throw `Required property "${keys[i]}" has not found in ${json[j]['reportingLabel']}.`;
            if (isEmpty(json[j]['size']))
                continue; // skip empty rows
            const value = json[j][keys[i]];
            if (isEmpty(value))
                throw `Required property "${keys[i]}" is empty in ${json[j]['reportingLabel']}.`;
        }
    }
    return true;
...
function getObjectByReportingLabel(data, reportingLabel) {
    for (let i = 0; i < data.length; i++) {
        if (data[i]['reportingLabel'] == reportingLabel)
            return data[i];
    }
    return null;
}
function fillElementByData(object, data) {
    const parent = object.children[0];
    for (const element of config.fillConfig) {
        if (element[2] == "image") {
            // get image
            if (data[element[1]] == '' || data[element[1]] == ' ' || data[element[1]] == undefined || data[element[1]] == null)
                continue;
...
function getVariant(data) {
    ...
    if (templatesPage == null)
        throw 'Can not find page with Templates. Please add it to your project.';
    ...
    if (templateSizeSet == null)
        throw `Can not find template set for size ${data['size']}.`;
    let variantQueue = '';
    ...
    if (variant == null)
        throw `Can not find variant {${variantQueue}} for size ${data['size']}.`;
    return variant;
}
